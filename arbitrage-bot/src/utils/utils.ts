...
export async function loadPair(pairId: string, mysql): Promise<Pair | null> {
  let table = getName(pairId) + 'pairs';
  const pair = await mysql.queryAsync(`SELECT * FROM ${table} WHERE id = ?`, [pairId]);
  return pair.length > 0 ? pair[0] : null;
}

export async function createPair(pair: Pair, mysql): Promise<void> {
  let table = getName(pair.id) + 'pairs';
  await mysql.queryAsync(`INSERT INTO ${table} SET ?`, [pair]);
}

export async function updatePair(pair: Pair, mysql): Promise<void> {
  let table = getName(pair.id) + 'pairs';
  await mysql.queryAsync(
    `UPDATE ${table} SET reserve0 = ?, reserve1 = ?, price = ?, timestamp = ?, synced = ?, tx = ? WHERE id = ?`,
    [pair.reserve0, pair.reserve1, pair.price, pair.timestamp, pair.synced, pair.tx, pair.id]
  );
}

export function getName(pair: string): string {
  console.log(pair);
  if (pair == process.env.SITSWAP_PAIR) return 'sithswap';
  else if (pair == process.env.JEDISWAP_PAIR) return 'jediswap';
  else if (pair == process.env.TENK_PAIR) return 'tenkswap';
  else {
    return 'null';
  }
}
